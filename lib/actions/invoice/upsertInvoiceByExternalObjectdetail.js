...
exports.process = async function ProcessAction(msg, cfg) {
    ...
    if (subsidiaryId) {
        const responce = await upsertExternalObject(instance, cfg, {
            subsidiary: {
                externalId: invoice.ClientID,
                name: invoice.ClientName,
                externalParentId: invoice.ClientID
            }
        }, cfg);

        subsidiaryId = responce.id;
    }
    ...
    if (helper.isEmpty(classificationId)) {
        const responce = await upsertExternalObject(instance, cfg, {
            classification: {
                externalId: invoice.PaymentCCID,
                name: invoice.PaymentCCID
            }
        });
        classificationId = responce.id;
    }

    if (helper.isEmpty(vendorId)) {
        const responce = await upsertExternalObject(instance, cfg, {
            vendor: {
                externalId: invoice.PayeeID,
                name: invoice.PayeeID
            }
        });
        vendorId = responce.id;
    }
    ...
    for (var i = 0; i < invoice.LineItems.length; i++) {
        ...
        if (helper.isEmpty(CostCenterID)) {
            const responce = await upsertExternalObject(instance, cfg, {
                classification: {
                    externalId: invoice.LineItems[i].CostCenterID,
                    name: invoice.LineItems[i].CostCenterID
                }
            });
            CostCenterID = responce.id;
        }

        if (helper.isEmpty(ExpenseGLAccountID)) {
            const responce = await upsertExternalObject(instance, cfg, {
                glAccount: {
                    externalId: invoice.LineItems[i].ExpenseGLAccountID,
                    name: invoice.LineItems[i].ExpenseGLAccountID
                }
            });
            ExpenseGLAccountID = responce.id;
        }
