...
exports.process = async function ProcessAction(msg, cfg) {
    ...
    if (Helper.isEmpty(body.bill.id) && vendorCreditAmount > 0) // Vendor credit only will create in the case of new bill posting
    {
        ...
        let vendorCreditResult = instance.makeRequest(`services/credit/${cfg.companyId}`, 'POST', vendorCreditBody);
        if (!Helper.isEmpty(vendorCreditResult))
        {
...
function Validate(bill) {
    ...
    if (utility.isEmpty(bill.term.id))
    {
    return new Error("Bill term id required");

    }
    if (utility.isEmpty(bill.classification.id))
    {
    return new Error("Bill classification id required");

    }
    if (utility.isEmpty(bill.vendor.id))
    {
