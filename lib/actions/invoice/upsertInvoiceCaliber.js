...
exports.process = async function ProcessAction(msg, cfg) {
    ...
    if (helper.isEmpty(invoice.ClientID)) { this.emit('error',('Subsidiary id is required'));}
    if (helper.isEmpty(classificationId)) { this.emit('error',('Classification id is required'));}
    if (helper.isEmpty(vendorId)) {this.emit('error',('Vendor id is required'));}
    if (helper.isEmpty(termId)) {this.emit('error',('Term Id id is required'));}
    if (helper.isEmpty(invoice.LineItems)) { this.emit('error',(' Atleast one line item is required')); }
    ...
    for (var i = 0; i < invoice.LineItems.length; i++) {
        ...
        if (helper.isEmpty(ExpenseGLAccountID)) {this.emit('error',('Expense GLAccountID is required'));}
        if (helper.isEmpty(CostCenterID)) {this.emit('error',('Expense CostCenterID is required'));}
        if (invoice.LineItems[i].Amount <= 0) {this.emit('error',('Expense line amount should be more than 0'));}
