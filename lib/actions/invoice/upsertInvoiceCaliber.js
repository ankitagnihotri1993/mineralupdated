...
exports.process = async function ProcessAction(msg, cfg) {
    ...
    for (var i = 0; i < invoice.LineItems.length; i++) {
        let CostCenterID = await GetSearchQuery(cfg,instance,"CLASS","dimension_externalId==" + invoice.LineItems[i].CostCenterID);
        let ExpenseGLAccountID = await GetSearchQuery(cfg,instance,"GLACCOUNT", "gl_account_number_externalId=="+invoice.LineItems[i].ExpenseGLAccountID);
