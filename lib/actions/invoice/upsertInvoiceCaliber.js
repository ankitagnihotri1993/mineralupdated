...
async function parseBody(result) {
...
exports.process = async function ProcessAction(msg, cfg) {
    ...
    const billId = parseBody(promiseResult[0]);
    const termId = parseBody(promiseResult[1]);
    const classificationId = parseBody(promiseResult[2]);
    const vendorId = parseBody(promiseResult[3]);
    ...
    for (var i = 0; i < invoice.LineItems.length; i++) {
        ...
        const CostCenterID = parseBody(result[0]);
        const ExpenseGLAccountID = parseBody(result[1]);
