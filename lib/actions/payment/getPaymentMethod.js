...
exports.process = async function ProcessAction(msg, cfg) {
  ...
  //  const responce = await instance.makeRequest(`services/pm/pmid`, 'GET');
     responce = await instance.makeRequest(`services/pm/pmid`, 'GET',msg.body);
    var result = '';
    ...
    if (responce && responce.companies) {
        ...
        if (companylist.length > 0) {
            ...
            if (company.length > 0) {
                ...
                const paymentMethod = company[0].paymentMethods.filter(res => res.externalId === model.value.externalId.toString());
