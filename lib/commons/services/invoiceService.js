...
const createPurchaseOrderURL = "/mtapi/base/services/purchaseorder/{companyId}";
...
module.exports = {
    ...
    createPurchaseOrder: async function (body, cfg, etag) {
        var URL = cfg.endPointURL + createPurchaseOrderURL.replace('{companyId}', cfg.companyId);
        return await request.post(util.requestOptions(URL, body, etag)).then(util.autoParse).catch(util.error);
    }
};