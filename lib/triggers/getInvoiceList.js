...
function processTrigger(msg, cfg,snapshot) 
{
    ...
    return accountService.login(cfg).then(function (response) {
        ...
        snapshot.lastMaxDate =  snapshot.lastMaxDate ||  ""

        var body=
        {
        body:{
            "view":"BILL",
            "query":"modified= ge="+filterDate.toISOString() //2018-08-16T13:10:57Z//+filterDate.toLocaleString()
       }};
        ...
        var modifiedObj = _.max(response.entities, function (invoice) {
            return   new Date(invoice.modified).getTime();
          });
