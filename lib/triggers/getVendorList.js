...
function processTrigger(msg, cfg,snapshot) 
{
    ...
    return accountService.login(cfg).then(function (response) {
        ...
        return vendorService.getVendorList(cfg, etag).then(function (response) {
            ...
            snapshot.vendorMaxLastDate = snapshot.vendorMaxLastDate ||  date.toLocaleString();;

            if (snapshot.vendorMaxLastDate != "") {

                var filterData = response.entities.filter(res => (new Date(res.modified) > new Date(snapshot.vendorMaxLastDate)));
                result = filterData;
            } else {
        ...
        snapshot.vendorMaxLastDate=vendorModifiedObj.modified;
        ...
        console.log('snapshot.vendorMaxLastDate', snapshot.vendorMaxLastDate);
